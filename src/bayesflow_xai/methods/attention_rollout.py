...
In plain terms: Attention Rollout measures which input time steps a
transformer actually "looked at" to build its summary. It multiplies the
attention weights from every layer together (mixing in a residual/identity
term per layer, since each layer also passes its input straight through),
so the result approximates how information from each time step flows all
the way to the output through every layer, not just the last one.
"""
...
from bayesflow_xai.utils.config import CONFIG, PARAM_NAMES, DEVICE, set_seed
...
def attention_rollout_generic(spec, body, head, X_val, y_val, target_idx, target_name,
                               fig_prefix: str = "workflow", **kwargs):
    """Simulator-agnostic entry point used by XAI_METHODS["attention_rollout"]
    (see bayesflow_xai.registrations). Requires `body` to be a trained
    TransformerSummaryNet (i.e. summary_network: transformer in the
    config) -- attention rollout is meaningless for a GRU/CNN body, which
    has no attention weights to roll out. `head`/`target_idx` are accepted
    for signature parity with the other XAI methods but unused: rollout
    describes which inputs the *encoder* attended to, independent of which
    output parameter is later read off."""
    if not isinstance(body, TransformerSummaryNet):
        raise ValueError(
            "xai_method 'attention_rollout' requires summary_network: transformer "
    ...
    with torch.no_grad():
        _ = body(X_val)
    rollout = attention_rollout(body.attn_maps)
    rollout_matrix = rollout.mean(dim=0).cpu().numpy()

    fig, ax = plt.subplots(figsize=(6, 5))
    im = ax.imshow(rollout_matrix, cmap="viridis")
    ax.set_title(f"Attention rollout matrix ({spec.name}, target={target_name})")
    ax.set_xlabel("Key step")
    ax.set_ylabel("Query step")
    fig.colorbar(im, ax=ax)
    show_and_save(fig, f"{fig_prefix}_attention_rollout", "Attention Rollout (generic)")

    return fig, rollout_matrix
