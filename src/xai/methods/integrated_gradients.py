...
In plain terms: Integrated Gradients measures each input's contribution by
walking a straight-line path from a baseline input (all zeros here) to the
real input, step by step, accumulating the gradient of the output at every
step along the way. Averaging those gradients and multiplying by the total
input change gives an attribution per input value that (unlike a single
raw gradient) accounts for the whole path, not just the local slope at the
real input.

`integrated_gradients_generic()` at the bottom of this module is the
simulator-agnostic entry point used by the registry-driven workflow
(scripts/run_workflow.py) -- everything above it is the original
SIR-specific analysis used by scripts/run_xai.py, unchanged.
"""
...
from xai.utils.config import CONFIG, DEVICE
from xai.simulation.sir.sir_model import prior, stationary_SIR
from xai.simulation.dataset import ensure_sir_fraction_scale
from xai.utils.plotting import show_and_save
from xai.methods.surrogate_models import train_sir_surrogate, TargetWrapper
...
def compute_channel_and_time_importance_stats(
    ...
    methods/surrogate_models.py) -- this is the actual coupling point to a
...
def integrated_gradients_generic(
    spec,
    body,
    head,
    X_val,
    ...
    `SimulatorSpec` (see xai.registry), timeseries or image, given an
    already-trained (body, head) surrogate. Used by
    XAI_METHODS["integrated_gradients"] (see xai.registrations)."""
    from captum.attr import IntegratedGradients
    from xai.methods.generic_surrogate import TargetWrapper
    ...
    if spec.input_kind == "timeseries":
        channel_importance = abs_attr.sum(axis=1).mean(axis=0)  # (C,)
        fig, ax = plt.subplots(figsize=(7, 4))
        ax.bar(spec.channel_names, channel_importance, color="#333333")
        ax.set_ylabel("Sum |attribution| over time")
        ax.set_title(f"IG channel importance ({spec.name}, target={target_name})")
    else:
        mean_attr = abs_attr.mean(axis=0)[0]  # (H, W)
        fig, ax = plt.subplots(figsize=(5, 5))
        im = ax.imshow(mean_attr, cmap="hot")
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        ax.set_title(f"IG pixel attribution ({spec.name}, target={target_name})")

    show_and_save(fig, f"{fig_prefix}_integrated_gradients", "Integrated Gradients (generic)")
    return fig, attributions, delta
