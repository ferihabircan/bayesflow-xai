...
dot-pixel-saliency map in methods/integrated_gradients.py but for a
...
same Captum-compatibility reason documented in methods/surrogate_models.py."""
...
from xai.simulation.grf.grf_model import prior, likelihood, FIELD_SHAPE, PARAM_NAMES
from xai.utils.config import DEVICE, set_seed
from xai.utils.plotting import show_and_save
from xai.methods.surrogate_models import TargetWrapper
...
def train_grf_surrogate(n_sims: int, epochs: int = 30, batch_size: int = 32, seed: int = 42):
    ...
    mirroring methods/surrogate_models.py:train_sir_surrogate."""
    set_seed(seed)
