"""Built-in plugin registrations for xai.registry.
...
`xai.registry` and calls its `register_*` decorators (see
`xai.registry`'s module docstring for a worked example), then import
...
def _build_lotka_volterra_spec() -> SimulatorSpec:
    from xai.simulation.dataset import build_lv_tensor_dataset
...
def _build_grf_spec() -> SimulatorSpec:
    from xai.methods.grf_xai import build_grf_tensor_dataset
    from xai.simulation.grf.grf_model import FIELD_SHAPE
...
def _build_gru_summary_network(spec: SimulatorSpec, hidden: int = 64, summary_dim: int = 8):
    from xai.methods.generic_surrogate import GenericGRUSummaryNet
...
def _build_conv_summary_network(spec: SimulatorSpec, widths=(8, 16, 32, 64), summary_dim: int = 6):
    from xai.methods.generic_surrogate import GenericCNNSummaryNet
...
def _build_transformer_summary_network(
    ...
    from xai.methods.attention_rollout import TransformerSummaryNet
...
def _xai_integrated_gradients(spec, body, head, X_val, y_val, target_idx, target_name, fig_prefix, **kwargs):
    from xai.methods.integrated_gradients import integrated_gradients_generic
...
def _xai_saliency_map(spec, body, head, X_val, y_val, target_idx, target_name, fig_prefix, **kwargs):
    from xai.methods.saliency_map import saliency_map_generic
...
def _xai_attention_rollout(spec, body, head, X_val, y_val, target_idx, target_name, fig_prefix, **kwargs):
    from xai.methods.attention_rollout import attention_rollout_generic
...
def _xai_masking(spec, body, head, X_val, y_val, target_idx, target_name, fig_prefix, **kwargs):
    from xai.methods.masking import masking_importance
