...
def set_seed(seed: int = 42) -> None:
    ...
    from xai.simulation.sir import sir_model
    ...
    # reached them via the new (xai.simulation.grf.*) or the
    # backward-compat flat (xai.simulation.grf_model) import path --
    # both populate the same sys.modules entry below.
    grf_model = sys.modules.get("xai.simulation.grf.grf_model")
    if grf_model is not None:
        grf_model.rng = np.random.default_rng(seed)

    grf_generative_model = sys.modules.get("xai.simulation.grf.grf_generative_model")
